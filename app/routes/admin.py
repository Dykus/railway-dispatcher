...
def create_backup():
    if request.user_role != 'admin': return "Доступ запрещён", 403
    try:
...
def list_backups():
    ...
    for path in all_backups:
        ...
        backups_info.append({'name': os.path.basename(path), 'rel_path': rel_path, 'size': stat.st_size, 'modified': datetime.fromtimestamp(stat.st_mtime).strftime('%Y-%m-%d %H:%M:%S')})
...
def download_backup():
    ...
    if not rel_path: return "Не указан путь", 400
    full_path = os.path.abspath(os.path.join(BACKUP_DIR, rel_path))
    if not full_path.startswith(os.path.abspath(BACKUP_DIR)): return "Неверный путь", 403
    if not os.path.exists(full_path): return "Файл не найден", 404
...
def restore_backup():
    ...
    full_path = os.path.abspath(os.path.join(BACKUP_DIR, rel_path))
    if not full_path.startswith(os.path.abspath(BACKUP_DIR)): return "Неверный путь", 403
    if not os.path.exists(full_path): return f"Файл не найден: {full_path}", 404
    try:
...
def view_logs():
    ...
    logs = c.fetchall()
    conn.close()
    action_translation = {'add': 'Добавление вагона', 'move': 'Перемещение', 'depart': 'Архивация', 'backup_create': 'Создание бэкапа', 'backup_restore': 'Восстановление из бэкапа', 'ip_user_edit': 'Изменение привязки IP', 'ip_user_delete': 'Удаление привязки IP', 'edit': 'Редактирование вагона', 'backup_auto': 'Автоматический бэкап', 'edit_history': 'Редактирование истории', 'track_add': 'Добавление пути', 'track_edit': 'Редактирование пути', 'track_delete': 'Удаление пути', 'track_move': 'Изменение порядка путей', 'track_reorder': 'Сохранение порядка путей', 'backup_remote': 'Удалённый бэкап', 'backup_remote_error': 'Ошибка удалённого бэкапа'}
...
def export_logs_excel():
    ...
    station_id = g.get('station_id', 1)
    conn = get_conn()
    df = pd.read_sql_query("SELECT timestamp as 'Время', username as 'Пользователь', ip_address as 'IP-адрес', action as 'Действие', wagon_number as 'Номер вагона', details as 'Детали', old_value as 'Старое значение', new_value as 'Новое значение' FROM action_log WHERE station_id = ? ORDER BY timestamp DESC", conn, params=(station_id,))
    conn.close()
    action_map = {'add': 'Добавление вагона', 'move': 'Перемещение', 'depart': 'Архивация', 'backup_create': 'Создание бэкапа', 'backup_restore': 'Восстановление из бэкапа', 'ip_user_edit': 'Изменение привязки IP', 'ip_user_delete': 'Удаление привязки IP', 'edit': 'Редактирование вагона', 'backup_auto': 'Автоматический бэкап', 'edit_history': 'Редактирование истории', 'track_add': 'Добавление пути', 'track_edit': 'Редактирование пути', 'track_delete': 'Удаление пути', 'track_move': 'Изменение порядка путей', 'track_reorder': 'Сохранение порядка путей', 'backup_remote': 'Удалённый бэкап', 'backup_remote_error': 'Ошибка удалённого бэкапа'}
...
def manage_ip_users():
    ...
    if request.method == 'POST':
        ...
        if ip and username:
            c.execute("INSERT OR REPLACE INTO ip_users (ip_address, username, note, access_allowed, role) VALUES (?, ?, ?, ?, ?)", (ip, username, note, access_allowed, role))
            conn.commit()
...
def edit_wagon_route(wagon_id):
    if request.user_role not in ('supervisor', 'admin'): return jsonify({"error": "Недостаточно прав"}), 403
    station_id = g.get('station_id', 1)
    ...
    else: return jsonify({"success": False, "message": msg}), 400
...
def edit_history(history_id):
    ...
    if not new_timestamp_str: return jsonify({"error": "Дата не указана"}), 400
    try:
    ...
    c.execute("SELECT id, timestamp FROM movement_history WHERE wagon_number = ? AND id != ? ORDER BY timestamp ASC", (wagon_num, history_id))
        ...
        if action_type == 'added':
            ...
            if loc and loc[0]:
                try:
                    ...
                    c.execute("UPDATE wagon_visits SET local_departure_time = ? WHERE wagon_number = ? AND is_archived = 0", (new_loc_dt.strftime('%Y-%m-%d %H:%M:%S'), wagon_num))
                except: pass
    conn.commit()
    log_action('edit_history', wagon_number=wagon_num, details=f"Изменена дата события #{history_id} с {old_timestamp} на {new_timestamp}", old_value=old_timestamp, new_value=new_timestamp)
    conn.close()
    return jsonify({"success": True, "message": "Дата успешно обновлена"})

@admin_bp.route('/changelog')
def changelog():
    if request.user_role != 'admin': return "Доступ запрещён. Список изменений доступен только администраторам.", 403
    if not os.path.exists(CHANGELOG_PATH): return f"Файл CHANGELOG.txt не найден. Ожидаемый путь: {CHANGELOG_PATH}", 404
...
def settings():
    ...
    if request.method == 'POST':
        ...
        if action == 'add_track':
            ...
            else: flash("Название и длина обязательны", 'error')
            ...
            global_keys = ['port', 'secret_key', 'backup_hour', 'backup_keep_count', 'remote_enabled', 'remote_path', 'remote_user', 'remote_password', 'log_max_mb', 'log_backup_count']
            for key in global_keys:
                if key == 'remote_enabled':
                    value = '1' if request.form.get(key) else '0'
                elif key == 'remote_password':
                    raw_val = request.form.get(key, '')
                    if raw_val: value = raw_val
                    else: continue
                else: value = request.form.get(key, '')
            ...
            if prog == '1':
                fine_keys = {'overstay_day1_rate': 1000, 'overstay_day2_rate': 2000, 'overstay_day3_rate': 4000, 'overstay_day4_plus_rate': 4000}
            else:
                fine_keys = {'overstay_fixed_rate': 2000}
...
def save_tracks_order():
    if request.user_role != 'admin': return jsonify({"error": "Доступ запрещён"}), 403
    data = request.get_json()
    if not data or 'order' not in data: return jsonify({"error": "Неверные данные"}), 400
    order = data['order']
    conn = get_conn()
    c = conn.cursor()
    try:
...
def rename_station():
    ...
    if not station_id or not new_name: return jsonify({"error": "Неверные данные"}), 400
    conn = get_conn()
    c = conn.cursor()
