...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS movement_history (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, wagon_number TEXT, action_type TEXT, 
                    from_track TEXT, to_track TEXT, note TEXT, timestamp TEXT, station_id INTEGER DEFAULT 1)''')
    c.execute('''CREATE TABLE IF NOT EXISTS archived_history (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, wagon_number TEXT, action_type TEXT, 
                    from_track TEXT, to_track TEXT, note TEXT, timestamp TEXT, archived_date TEXT,
                    visit_id INTEGER DEFAULT NULL, station_id INTEGER DEFAULT 1)''')
    
    for sql in [
        "ALTER TABLE movement_history ADD COLUMN station_id INTEGER DEFAULT 1",
        "ALTER TABLE archived_history ADD COLUMN station_id INTEGER DEFAULT 1",
        "ALTER TABLE archived_history ADD COLUMN visit_id INTEGER DEFAULT NULL"
    ]:
        try: c.execute(sql)
        except: pass

    c.execute('''CREATE TABLE IF NOT EXISTS tracks (
                    id INTEGER PRIMARY KEY, name TEXT NOT NULL, total_length REAL, 
                    track_type TEXT DEFAULT 'normal', sort_order INTEGER DEFAULT 0, 
                    station_id INTEGER DEFAULT 1, UNIQUE(name, station_id))''')
    try: c.execute("ALTER TABLE tracks ADD COLUMN station_id INTEGER DEFAULT 1")
    except: pass

    c.execute('''CREATE TABLE IF NOT EXISTS wagons (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, wagon_number TEXT UNIQUE, length REAL, 
                    cargo_type TEXT, owner TEXT, organization TEXT, status TEXT DEFAULT 'assigned', 
                    track_id INTEGER, start_pos REAL, arrival_time TEXT, departure_time TEXT, 
                    local_departure_time TEXT, visit_count INTEGER DEFAULT 0, is_archived INTEGER DEFAULT 0)''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS wagon_visits (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, wagon_number TEXT NOT NULL, arrival_time TEXT,
                    departure_time TEXT, local_departure_time TEXT, is_archived INTEGER DEFAULT 0,
                    track_id INTEGER, start_pos REAL, cargo_type TEXT, owner TEXT, organization TEXT,
                    length REAL DEFAULT 10.0, status TEXT DEFAULT 'assigned', visit_count INTEGER DEFAULT 0,
                    station_id INTEGER DEFAULT 1)''')
    try: c.execute("ALTER TABLE wagon_visits ADD COLUMN station_id INTEGER DEFAULT 1")
    except: pass

    c.execute('''CREATE TABLE IF NOT EXISTS action_log (
                    id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT, username TEXT, 
                    ip_address TEXT, action TEXT, wagon_number TEXT, details TEXT, 
                    old_value TEXT, new_value TEXT, station_id INTEGER DEFAULT 1)''')
    try: c.execute("ALTER TABLE action_log ADD COLUMN station_id INTEGER DEFAULT 1")
    except: pass

    c.execute('''CREATE TABLE IF NOT EXISTS ip_users (
                    ip_address TEXT PRIMARY KEY, username TEXT, note TEXT, 
                    is_admin INTEGER DEFAULT 0, role TEXT DEFAULT 'dispatcher', 
                    access_allowed INTEGER DEFAULT 0)''')
    for sql in [
        "ALTER TABLE ip_users ADD COLUMN is_admin INTEGER DEFAULT 0",
        "ALTER TABLE ip_users ADD COLUMN role TEXT DEFAULT 'dispatcher'",
        "ALTER TABLE ip_users ADD COLUMN access_allowed INTEGER DEFAULT 0"
    ]:
        try: c.execute(sql)
        except: pass
    ...
    if c.fetchone()[0] == 0:
        default_settings = [
            ('port', '5000', 0), ('secret_key', 'rail_app_secret_key_change_me', 0),
            ('backup_hour', '3', 0), ('backup_keep_count', '30', 0),
            ('remote_enabled', '0', 0), ('remote_path', '', 0),
            ('remote_user', '', 0), ('remote_password', '', 0),
            ('log_max_mb', '5', 0), ('log_backup_count', '5', 0),
            ('refresh_interval', '5', 0), ('theme', 'light', 0),
            ('default_wagon_length', '10.0', 0), ('wagon_spacing', '50.0', 0)
        ]
    ...
    if c.fetchone()[0] == 0:
        ...
        data2 = [(t[0]+8, t[1], t[2], t[3], t[4], 2) for t in data]
...
def migrate_to_visits():
    conn = get_conn()
    c = conn.cursor()
    try: c.execute("ALTER TABLE archived_history ADD COLUMN visit_id INTEGER DEFAULT NULL")
    except: pass
    c.execute("SELECT COUNT(*) FROM wagon_visits")
    if c.fetchone()[0] == 0:
        c.execute("""INSERT INTO wagon_visits (wagon_number, arrival_time, departure_time, local_departure_time,
                     ...
                     SELECT wagon_number, arrival_time, departure_time, local_departure_time,
                            is_archived, track_id, start_pos, cargo_type, owner, organization, length, status, visit_count, 1 FROM wagons""")
        conn.commit()
    c.execute("""UPDATE archived_history SET visit_id = (
                    SELECT wv.id FROM wagon_visits wv WHERE wv.wagon_number = archived_history.wagon_number
                    AND wv.is_archived = 1 AND wv.arrival_time <= archived_history.timestamp ORDER BY wv.arrival_time DESC LIMIT 1) WHERE visit_id IS NULL""")
    conn.commit()
    conn.close()
...
def migrate_to_stations():
    ...
    if 'station_id' not in cols:
        for table in ['tracks', 'wagon_visits', 'movement_history', 'archived_history', 'action_log']:
            try: c.execute(f"ALTER TABLE {table} ADD COLUMN station_id INTEGER DEFAULT 1")
            except: pass
        for table in ['tracks', 'wagon_visits', 'movement_history', 'archived_history', 'action_log']:
...
def get_last_auto_backup_time():
    ...
    if not os.path.exists(auto_dir): return None
...
def copy_backup_to_network(backup_filename):
    ...
    try:
        if remote_user and remote_password:
            os.system(f'net use "{remote_path}" /user:{remote_user} {remote_password} >nul 2>&1')
    ...
    if manual_start_str and manual_start_str.strip():
        try:
            ...
            if last_event_dt and start_dt <= last_event_dt:
                conn.close()
                return False, "Дата начала отсчёта не может быть раньше или равна предыдущему событию"
            if arrival_time_str:
                try:
                    arrival_dt_check = datetime.strptime(arrival_time_str, '%Y-%m-%d %H:%M:%S')
                    if start_dt <= arrival_dt_check:
                        conn.close()
                        return False, "Дата начала отсчёта не может быть раньше времени прибытия"
                except:
                    pass
...
def edit_wagon(visit_id, new_owner=None, new_org=None, new_note=None, new_arrival_time=None, new_global_deadline=None, new_local_deadline=None, station_id=1):
    conn = get_conn()
    c = conn.cursor()
    c.execute("""SELECT wagon_number, owner, organization, cargo_type, arrival_time, departure_time, local_departure_time, track_id
    ...
    if new_owner is not None and new_owner != old_owner:
        updates.append("owner = ?"); params.append(new_owner); changes.append(f"ТК: '{old_owner}' → '{new_owner}'")
    if new_org is not None and new_org != old_org:
        updates.append("organization = ?"); params.append(new_org); changes.append(f"Организация: '{old_org}' → '{new_org}'")
    if new_note is not None and new_note != old_note:
        updates.append("cargo_type = ?"); params.append(clean_note_for_db(new_note)); changes.append(f"Примечание: '{old_note}' → '{new_note}'")
    ...
    if new_arrival_time is not None and new_arrival_time.strip() != "":
        try:
            ...
            if new_arr_dt is None: raise ValueError("Пустая дата")
            new_arrival_time_str = new_arr_dt.strftime('%Y-%m-%d %H:%M:%S')
        except ValueError as e:
            conn.close(); return False, f"Ошибка в дате прибытия: {e}"
        if last_event_dt and new_arr_dt > last_event_dt:
            conn.close(); return False, "Дата прибытия не может быть позже последнего перемещения"
        updates.append("arrival_time = ?"); params.append(new_arrival_time_str); changes.append(f"Время прибытия: '{old_arrival}' → '{new_arrival_time_str}'")

    if new_global_deadline is not None and new_global_deadline.strip() != "":
        try:
            from app.utils import parse_flexible_date
            new_glob_dt = parse_flexible_date(new_global_deadline)
            if new_glob_dt is None: raise ValueError("Пустая дата")
            new_global_str = new_glob_dt.strftime('%Y-%m-%d %H:%M:%S')
        except ValueError as e:
            conn.close(); return False, f"Ошибка в глобальном сроке: {e}"
        arrival_dt = datetime.strptime(old_arrival, '%Y-%m-%d %H:%M:%S') if old_arrival else None
        if arrival_dt and new_glob_dt < arrival_dt:
            conn.close(); return False, "Глобальный срок не может быть раньше времени прибытия"
        if last_event_dt and new_glob_dt < last_event_dt:
            conn.close(); return False, "Глобальный срок не может быть раньше последнего перемещения"
        updates.append("departure_time = ?"); params.append(new_global_str); changes.append(f"Глобальный срок: '{old_global}' → '{new_global_str}'")

    if new_local_deadline is not None and new_local_deadline.strip() != "":
        try:
            from app.utils import parse_flexible_date
            new_local_dt = parse_flexible_date(new_local_deadline)
            if new_local_dt is None: raise ValueError("Пустая дата")
            new_local_str = new_local_dt.strftime('%Y-%m-%d %H:%M:%S')
        except ValueError as e:
            conn.close(); return False, f"Ошибка в локальном сроке: {e}"
        arrival_dt = datetime.strptime(old_arrival, '%Y-%m-%d %H:%M:%S') if old_arrival else None
        if arrival_dt and new_local_dt < arrival_dt:
            conn.close(); return False, "Локальный срок не может быть раньше времени прибытия"
        if last_event_dt and new_local_dt < last_event_dt:
            conn.close(); return False, "Локальный срок не может быть раньше последнего перемещения"
        updates.append("local_departure_time = ?"); params.append(new_local_str); changes.append(f"Локальный срок: '{old_local}' → '{new_local_str}'")

    if not updates:
        conn.close(); return True, "Нет изменений"
...
def get_dashboard_data(station_id=1):
    ...
    for w in all_wagons_raw:
        tid = w[6]
        if tid not in wagons_by_track: wagons_by_track[tid] = []
        wagons_by_track[tid].append(w)

    for t_id, t_name, t_len, t_type in tracks_raw:
        try: t_len = float(t_len)
        except: t_len = 1000.0
        ...
        for w in track_wagons:
            ...
            try: w_len = float(w_len) if w_len is not None else 10.0
            except: w_len = 10.0
            ...
            if w_loc:
                try: 
                    ...
                    if diff > 0:
                        m, s = divmod(int(diff), 60); h, m = divmod(m, 60); d, h = divmod(h, 24)
                        loc_text_parts = {"raw": diff, "d": d, "h": h, "m": m, "s": s, "iso": loc_iso, "overdue": False}
                    else:
            ...
            if w_glob:
                try: 
                    ...
                    if diff > 0: 
                        m, s = divmod(int(diff), 60); h, m = divmod(m, 60); d, h = divmod(h, 24)
                        glob_text_parts = {"raw": diff, "d": d, "h": h, "m": m, "iso": glob_iso, "overdue": False}
                    else: 
            ...
            processed.append({
                "id": w_id, "num": w_num, "note": w_note or "-", "owner": w_owner or "Не указана", 
                "org": w_org or "Не указано", "pos": w_pos, "arrival": format_date(w_arr),
                "loc": loc_text_parts, "glob": glob_text_parts,
...
def get_grouped_history(station_id=1):
    ...
    c.execute("""SELECT m.id, m.wagon_number, m.action_type, m.from_track, m.to_track, m.note, m.timestamp, wv.owner, wv.organization, wv.cargo_type
                 FROM movement_history m LEFT JOIN wagon_visits wv ON m.wagon_number = wv.wagon_number AND wv.is_archived = 0 AND wv.station_id = ?
                 WHERE m.station_id = ? ORDER BY m.wagon_number, m.timestamp ASC""", (station_id, station_id))
    rows = c.fetchall()
    conn.close()
    ...
    for w_num in sorted_wagons:
        ...
        for idx, ev in enumerate(events): ev['is_last'] = (idx == len(events) - 1)
        result.append({"num": w_num, "last_status": events[-1]['action'], "last_time": events[-1]['time'], "events": events, "count": len(events)})
    return result
...
def get_grouped_archive_history(station_id=1):
    ...
    c.execute("""SELECT a.wagon_number, a.action_type, a.from_track, a.to_track, a.note, a.timestamp, wv.owner, wv.organization, wv.cargo_type
                 FROM archived_history a LEFT JOIN wagon_visits wv ON a.visit_id = wv.id
                 WHERE a.station_id = ? ORDER BY a.wagon_number, a.timestamp ASC""", (station_id,))
    rows = c.fetchall()
    conn.close()
    grouped = defaultdict(list)
    for row in rows:
        ...
        grouped[w_num].append({"action": action_label, "from": from_t or "-", "to": to_t or "-", 
                               "owner": owner or "-", "org": org or "-", "cargo": cargo_type or "-", "note": note or "-", "time": ts_str})
    def sort_key(k):
        try: return (0, int(k))
        except: return (1, k)
